...
function verifyV1Style(rawBody, header) {
    ...
    const tolSec = Number.isFinite(parseInt(process.env.WEBHOOK_TOLERANCE_SEC, 10))
        ? parseInt(process.env.WEBHOOK_TOLERANCE_SEC, 10)
        : 300;
    ...
    if (!secrets.length) return true; // no secret configured; allow but warn upstream

    const payloadToSign = Buffer.concat([Buffer.from(String(t)), Buffer.from('.'), Buffer.from(rawBody || '')]);
...
function verifyZohoHeader(rawBody, header) {
    // x-zoho-signature: base64 or hex of HMAC(rawBody)
