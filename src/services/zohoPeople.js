...
function officialEmailFromUpn(upn) {
  // if OFFICIAL_EMAIL_DOMAIN is set, rewrite domain for “Official Email” (e.g. roundglass.com)
...
async function updateCandidateOfficialEmail({ recordId, officialEmail, fieldLinkName }) {
  ...
  // Zoho expects numeric recordId (long number). Enforce/Fail early.
  if (!/^\d{8,}$/.test(id)) {
  ...
  // Use the /people/api path per Zoho doc
  ...
  const res = await axios.post(url, body, {
    ...
    validateStatus: () => true, // let us handle Zoho error payloads
  });

  // Zoho returns { response: { status: 0 } } for success;
  // when there’s an error, response.status = 1 with an "errors" object.
  const data = res.data;
  const status = data?.response?.status;

  if (status === 0) return data;

  const zErr = data?.response?.errors || data;
  const msg = JSON.stringify(zErr);
  throw new Error(`Zoho update failed: ${msg}`);
}
...
/**
 * Scan Employees in Zoho People and return (max numeric Employee ID + 1) as a string.
 * Paginates with slindex/rec_limit to avoid missing older employees.
 */
async function getNextEmployeeIdFromZoho({ pageSize = 200, maxPages = 50 } = {}) {
  ...
  return String(maxNum + 1 || 1); // start at "1" if none found
}
...
async function getLastEmployeeIdFromZoho() {
  ...
  // Use Employee view; ask one record, sorted by Employee_ID desc
  ...
  // Defensive parsing (Zoho response shapes can vary slightly)
  const records =
