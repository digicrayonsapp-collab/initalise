...
async function graphRequest(method, url, token, data) {
  const res = await graphHttp.request({
    method,
    url,                              // absolute or relative both OK
    data: data ?? undefined,          // undefined for body-less calls
    headers: { Authorization: `Bearer ${token}` },
  });
  return res.data;
}
...
async function findByEmployeeId(token, employeeId) {
  ...
  const data = await graphGet(token, "/users", {
    ...
    $select: "id,userPrincipalName,mail,employeeId,accountEnabled,displayName,otherMails", // 👈 added otherMails
  });
  return (data.value && data.value[0]) || null;
}
...
async function getNextEmployeeId(token, { maxScan = 50000 } = {}) {
  ...
  while (url) {
    const res = await graphHttp.get(url, {
      headers: { Authorization: `Bearer ${token}` },
    });
    ...
    for (const u of batch) {
      ...
      if (!raw) continue;                 // skip null/empty
      const m = raw.match(/\d+/);         // take the leading number portion
      if (!m) continue;
      const n = parseInt(m[0], 10);
      if (Number.isFinite(n) && n > maxNum) maxNum = n;
    }

    // nextLink may be absolute; our axios instance handles both
    url = res.data?.["@odata.nextLink"] || null;

    if (scanned >= maxScan) break;        // safety cap
  }
...
async function findUserByDisplayName(token, displayName) {
  try {
    ...
    const url = `https://graph.microsoft.com/v1.0/users?$filter=displayName eq '${encodedName}'`;
    ...
    if (response.data && response.data.value && response.data.value.length > 0) {
      return response.data.value[0]; // return first match
    }
...
module.exports = {
  ...
  findUserByDisplayName  // export in case route wants to use it
};
